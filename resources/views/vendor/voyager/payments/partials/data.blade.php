                ...
                @foreach($dataType->browseRows as $row)
                    ...
                    @if ($row->field == 'status')
                        <td>
                            ...
                            <p>Username: @include('voyager::payments.username')</p>
                            <p>Số xu: @include('voyager::payments.gamecoin')</p>
                        </td>
                    @endif
                @endforeach
            </tr>
            <tr class="xs-actions {{ $k % 2 == 0 ? 'default' : 'active' }}">
                ...
                <td colspan="2" class="no-sort no-click bread-actions">
                    @foreach(array_reverse(Voyager::actions()) as $action)
                        @include(Voyager::getBreadView('partials.actions-xs', $dataType->slug), ['action' => $action])
                    @endforeach
                </td>
            </tr>
        @endforeach
